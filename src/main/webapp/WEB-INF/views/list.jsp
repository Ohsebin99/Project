								...
								<div class="slider-item-content">
									<div class="post-thumb mb-4">
										<a class="post-thumb mb-4 d-block" href="selectByIdx?idx=${so.idx}&currentPage=${currentPage}&job=reple">
											<img src="upload/thumbnail/${so.thumbnail}" class="img-fluid" style="height: 500px; border-radius: 20px;">
										</a>
									</div>

									<div class="slider-post-content">
										<span class="text-color font-sm font-extra letter-spacing">${so.category}</span>
										<h3 class="post-title mt-1">
											<a href="selectByIdx?idx=${so.idx}&currentPage=${currentPage}&job=reple">${so.subject}</a>
										</h3>
							...
							<form class="mb-5" action="list" method="post">
								...
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
									<div class="row mb-3">
								        ...
								        <!-- 검색 내용 -->
								        <div class="col-md-7"> <!-- col-md-7로 변경 -->
								            <div class="input-group"> <!-- input-group 추가 -->
								        ...
								        <div class="col-md-1"> <!-- col-md-1로 변경 -->
								        </div>
								    </div>
								</div>
							</form>
						...
						<ul class="list-inline">
							...
							<c:if test="${partyList.currentPage >1}">
								<li class="list-inline-item">
									<a class="active" href="?currentPage=1&partyGender=${partyGender}&item=${item}&condition=${condition}&acholchk=${acholchk}&location=${location}&category=${category}">처음</a>
								</li>
							</c:if>
							...
							<c:if test="${partyList.startPage >1}">
								<li class="list-inline-item">
									<a href="?currentPage=${partyList.startPage - 1}&partyGender=${partyGender}&item=${item}&condition=${condition}&acholchk=${acholchk}&location=${location}&category=${category}">이전</a>
								</li>
							</c:if>
							...
							<c:forEach var="i" begin="${partyList.startPage}" end="${partyList.endPage}" step="1">
								...
								<c:if test="${partyList.currentPage != i}">
									<li class="list-inline-item">
										<a href="?currentPage=${i}&partyGender=${partyGender}&item=${item}&condition=${condition}&acholchk=${acholchk}&location=${location}&category=${category}">${i}</a>
									</li>
								</c:if>
							</c:forEach>
							...
							<c:if test="${partyList.endPage < partyList.totalPage}">
								<li class="list-inline-item">
									<a	href="?currentPage=${partyList.endPage + 1}&partyGender=${partyGender}&item=${item}&condition=${condition}&acholchk=${acholchk}&location=${location}&category=${category}">다음</a>
								</li>
							</c:if>
							...
							<c:if test="${partyList.currentPage < partyList.totalPage}">
								<li class="list-inline-item">
									<a class="active" href="?currentPage=${partyList.totalPage}&partyGender=${partyGender}&item=${item}&condition=${condition}&acholchk=${acholchk}&location=${location}&category=${category}">끝</a>
								</li>
							</c:if>
						</ul>
					</div>
