	       	...
	       	<div class="post-header mb-5 text-center">
				...
				${subject}
	           	...
	           	<div class="post-featured-image mt-5">
					<img src="upload/thumbnail/${vo.thumbnail}" class="img-fluid w-100" style="height: 300px;"/>
	           	</div>
			</div>
			<div class="post-body">
				<!--  -->
	                       	...
	                       	<div class="col-lg-12 text-center" style="margin-bottom: 15px;">
								<div class="btn-group">
									...
									<input class="btn btn-primary" type="button" value="수정하기"
	                                	onclick="location.href='selectByIdx?idx=${vo.idx}&currentPage=${currentPage}&job=partyUpdate'"/>
	                                <input class="btn btn-primary" type="button" value="삭제하기"
	                                	onclick="location.href='selectByIdx?idx=${vo.idx}&currentPage=${currentPage}&job=partyDelete'"/>
	                               	</c:if>   
		        ...
		        <!-- 신고 -->
		       	<c:if test="${vo.id != mo.id && reportCount == 0}">
					<form action="report">
				...
				<!-- 참여 하기 -->
				<c:if test="${vo.id != mo.id}">
				<form action="repleInsert" method="post" onsubmit="return joinChk()">
		...
		<div class="sidebar sidebar-right">
            ...
            <div class="sidebar-widget about mb-5 text-center p-3">
               <div class="about-author">
                  <img src="upload/memberphoto/${vo.masterPhoto}" class="img-fluid">
               </div>
            ...
            <div class="sidebar-widget mb-5 ">
                ...
                <!-- 신청목록 -->
                <c:if test="${ro.repleId == mo.id || ro.masterId == mo.id && ro.fix == 'N'}">
					<div class="media border-bottom py-3 sidebar-post-item">
	                	<img class="mr-4" src="upload/memberphoto/${ro.replePhoto}">
	                    <div class="media-body">
	                    	<h5 class="widget">${ro.repleNickName}</h5>
	                        <span class="text-muted font-sm">
	                        	<fmt:formatDate value="${ro.writeDate}" pattern="MM월 dd일 HH:mm" />
							</span><br/>
	                        <span>${ro.repleGender}</span>
	                        <span>${ro.repleAge}</span>
                ...
                <c:if test="${ro.masterId == mo.id && ro.fix == 'N'}">
                	<input class="btn btn-primary" type="button" value="승인"
                    	onclick="join('승인', ${vo.idx}, ${currentPage}, ${ro.repleIdx})"/>
                  	<input class="btn btn-primary" type="button" value="거절"
                    	onclick="join('거절', ${vo.idx}, ${currentPage}, ${ro.repleIdx})"/>
				</c:if>
                </c:if>
                </c:forEach>
                 ...
                 <!-- 참여목록 -->
					...
					<div class="media border-bottom py-2 sidebar-post-item">
                        ...
                        <div class="media-body">
	                        ...
	                        <span>${ro.repleGender}</span>
	                        <span>${ro.repleAge}</span>
	                        <span class="font-sm letter-spacing-1">${ro.reple}</span>
	                        <!-- 참여자 파티 탈퇴 버튼 -->
                    	</div>
			            <c:if test="${ro.repleId == mo.id}">
		            		<input class="btn btn-primary m-3" type="button" value="취소"
		            			onclick="join('파티 취소', ${vo.idx}, ${currentPage}, ${ro.repleIdx})"/>
						</c:if>
                	</div>
                	</c:if>
				</c:forEach>
